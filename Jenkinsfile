    ...
    stages {
	    ...
	    stage ('Cleanup Artifacts') {
         ...
         stage("Trigger CD Pipeline") {
            steps {
                script {
                    sh "curl -v -k --user admin:${JENKINS_API_TOKEN} -X POST -H 'cache-control: no-cache' -H 'content-type: application/x-www-form-urlencoded' --data 'IMAGE_TAG=${IMAGE_TAG}' 'ec2-65-0-81-213.ap-south-1.compute.amazonaws.com:8080/job/auth-service-cd/buildWithParameters?token=gitops-token'"
                }
            }
         } 
     }
